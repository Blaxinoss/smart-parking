...
function RootLayoutNav() {
  ...
  useEffect(() => {
    ...
    if (!navigationState?.key) {
      console.log(
        "🛑 [Redirect Guard]: Navigation state is NOT ready yet. Skipping redirect.",
      );
      return;
    }
    ...
    if (!firebaseUser) {
      ...
      if (!inAuthGroup) {
        console.log(
          "➡️ [Redirecting]: Guest trying to access protected route. Moving to -> /(auth)/login",
        );
      ...
      console.log(
        "ℹ️ [Auth Status]: Firebase User exists, but Backend userData is missing.",
      );
      if (!inOnboardingStackGroup) {
        console.log(
          "➡️ [Redirecting]: User needs onboarding. Moving to -> /(onboard)",
        );
      ...
      if (
        inAuthGroup ||
        inOnboardingStackGroup ||
        (inTabs && userData.role === "ADMIN") ||
        (inAdmin && userData.role !== "ADMIN")
      ) {
        if (userData.role === "ADMIN") {
          console.log(
            "➡️ [Redirecting]: Admin found in wrong stack. Moving to -> /(admin)",
          );
          router.replace("/(admin)");
        } else {
          console.log(
            "➡️ [Redirecting]: Regular user found in wrong stack. Moving to -> /(tabs)",
          );
          router.replace("/(tabs)");
        }
      } else {
        console.log(
          `✅ [Stay]: User is in the correct group for role: ${userData.role}`,
        );
      }
    }
  }, [
  ...
  if (isFirebaseLoading || !navigationState?.key) {
    console.log(
      "⏳ [Render Guard A]: Freezing screen via ActivityIndicator because:",
      {
        isFirebaseLoading,
        isNavReady: !!navigationState?.key,
      },
  ...
  if (!firebaseUser && !inAuthGroup) {
    console.log(
      "⏳ [Render Guard B]: Freezing screen. Guest user trying to view protected segment:",
      segments[0],
    );
    return (
      <ActivityIndicator
  ...
  if (firebaseUser && isLoading && !inOnboardingStackGroup && !inAuthGroup) {
    console.log(
      "⏳ [Render Guard C]: Freezing screen. Waiting for backend useUser() data to load...",
    );
    return (
      <ActivityIndicator
        size="large"
  ...
  if (inAdmin && userData?.role !== "ADMIN") {
    console.log(
      `🚨 [Render Guard D]: Access Denied! Role [${userData?.role}] tried to access /(admin). Freezing until redirected.`,
    );
    return (
      <ActivityIndicator
        size="large"
  ...
  console.log(
    "✨ [SUCCESS RENDERING]: All guards passed, rendering <Slot /> now.",
  );
