...
export const LocationMapParent = ({
  setShouldSessionStart,
  bottomSheetRef,
}: {
  setShouldSessionStart: (x: boolean) => void;
  bottomSheetRef: React.RefObject<BottomSheet | null>;
}) => {
  const { coordsFull, setcoordsFull, initLocation, FIXED_MAYCHANGE_DEST } =
    useLocationHook();
  ...
  useEffect(() => {
    if (coordsFull && coordsFull.latitude && coordsFull.longitude) {
      const isInside = isPointWithinRadius(
        ...
        {
          latitude: FIXED_MAYCHANGE_DEST.latitude,
          longitude: FIXED_MAYCHANGE_DEST.longitude,
        },
        150,
      );
  ...
  if (!initLocation) {
    return (
      <View className="flex-1 justify-center items-center bg-garage-950">
        ...
        <StyledText className="text-garage-200 mt-2">
          جاري تحديد موقعك...
        </StyledText>
      </View>
    );
