    ...
    <View className="flex-1 w-full px-6 py-8">
      ...
      <View className="mb-8">
        <View className="flex-row justify-between items-start mb-6">
          ...
          {/* Timer */}
          {isTimerShown && (
            <View className="items-center bg-garage-900 px-4 py-3 rounded-2xl border-2 border-main-900">
        ...
        <View className="flex-row items-center gap-2 mb-2">
          <View
            className={`flex-1 h-2 ${isVerifying || isWaiting || isAllowed ? "bg-main-900" : "bg-garage-800 "} rounded-full`}
          />
          <View
            className={`flex-1 h-2 ${isAllowed || isDenied ? "bg-main-900" : "bg-garage-800 "} rounded-full* `}
          />
      ...
      <View className="items-center mb-5">
        <StyledText
          className={`text-3xl font-titillium-bold mb-8 text-center
        ...
        >
          {isVerifying
            ? "Verifying Plate..."
            : isAllowed
              ? "The gate is opened for you to enter!"
              : isDenied
                ? `Access denied with message ${gateMessage}`
        ...
        <View className="relative">
          {/* Outer Glow Ring */}
          <View
            className={`absolute inset-0 w-24 h-24 z-10 rounded-full ${isVerifying ? "bg-main-900/20" : isAllowed ? "bg-green-600/40" : isDenied ? "bg-red-700/40" : "bg-blue-500/20"} blur-2xl`}
          />

          {/* Main Scanner Circle */}
          <View
            className={`w-24 h-24 rounded-full items-center justify-center border-4 bg-garage-900 ${
              isVerifying
                ? "border-main-900"
                : isAllowed
                  ? "border-green-600"
            ...
            {isVerifying ||
              (isWaiting && (
                <Animated.View
                  ...
                  className={`absolute w-full h-1 shadow-2xl ${
                    isVerifying
      ...
      <View className="bg-blue-500/10 border-2 border-blue-500/30 rounded-2xl p-4 mt-4">
        <View className="flex-row items-center gap-3">
          ...
          <StyledText className="flex-1 text-blue-400 text-sm font-titillium-regular">
            {isVerifying
              ? "Please wait while we verify your plate."
              : isAllowed
                ? "Proceed safely to your assigned slot."
                : isDenied
                  ? "Please check your reservation or contact support."
